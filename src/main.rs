  ...
  fn plus_forte_que(&self, autre: &Carte, appelee: Couleur) -> bool {
    match self {
      Carte::CarteNorm(coul, val) => match autre {
        Carte::Atout(_) => false,
        Carte::CarteNorm(coul_autre, val_autre) => if *coul == *coul_autre {
...
fn a_couleur(cartes: &[Carte], couleur: Option<Couleur>) -> bool {
  for c in cartes {
    match c {
      Carte::CarteNorm(c, _) => if let Some(coul_demandee) = couleur.clone() {
        if *c == coul_demandee {
          return true;
        }
      },
      Carte::Atout(_) => if couleur.is_none() {
        return true;
      }
    }
  }
...
fn atout_max(cartes: &[Carte]) -> Option<Atout> {
  cartes.iter()
    .fold(None, |max, carte| match carte {
      Carte::CarteNorm(_, _) => max,
      Carte::Atout(at) => if let Some(at_max) = max.clone() {
        if *at > at_max {
          Some(at.clone())
        } else {
          max.clone()
...
fn gagnant_de_tour(cartes: &[(Joueur, Carte)]) -> Joueur {
  ...
  cartes.iter().fold(cartes[0].clone(), |gagnant, joueur| {
    if gagnant.1.plus_forte_que(&joueur.1, demandee.clone().unwrap_or(Couleur::Carreau)) {
      gagnant
    } else {
      joueur.clone()
    }
...
fn cartes_jouables(cartes_jouees: &[Carte], cartes_joueur: &[Carte], premier_tour: bool, roi_appele: Couleur) -> Vec<Carte> {
...
fn creer_jeu() -> Vec<Carte> {
  ...
  for coul in [Couleur::Carreau, Couleur::Coeur, Couleur::Trefle, Couleur::Pique].iter() {
    for val in [
      Valeur::As,
      Valeur::Deux,
      Valeur::Trois,
...
fn main() {
  ...
  let jouables = cartes_jouables(
    ...
    &[ jeu[1].clone(), jeu[20].clone(), jeu[50].clone(), jeu[60].clone(), ],
    false,
    Couleur::Carreau,
  );
  ...
  let jouables = cartes_jouables(
    &[ jeu[2].clone(), jeu[5].clone(), jeu[6].clone(), ],
    &[ jeu[1].clone(), jeu[20].clone(), jeu[50].clone(), jeu[60].clone(), jeu[77].clone() ],
    false,
    Couleur::Carreau,
  );
